...
/*   Updated: 2019/12/01 18:47:32 by bomanyte         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (input_not_over == PRO_SUBSH || input_not_over == PRO_MATH || input_not_over == PRO_PIPE
	|| input_not_over == PRO_AND || input_not_over == PRO_OR || input_not_over == PRO_SQU)
		return (1);
	return (0);
}
...
static short	is_tok_what(t_dlist *token_list, t_tk_type type)
{
	if (type == SEPS)
		...
		&& TOK_TYPE != TK_PIPE && TOK_TYPE != TK_BCKR_PS && TOK_TYPE != TK_ARSHLOCH)
			return (0);
	if (type == FLOWS)
...
static short	vector_to_left(t_dlist *token_list)
{
	...
	if (!token_list)
		return (1);
	if (token_list && !is_tok_what(token_list, SEPS) && !is_tok_what(token_list, FLOWS)
	&& TOK_TYPE != TK_DEREF && TOK_TYPE != TK_ASSIGM && TOK_TYPE != TK_FUNCTION &&
	!is_tok_redir(TOK_TYPE, 1) && TOK_TYPE != TK_EXEC && TOK_TYPE != TK_RETURN)
		return (0);
	return (1);
}
...
static short	vector_to_right(t_dlist *token_list)
{
	...
	&& !is_tok_redir(TOK_TYPE, 1) && !is_tok_what(token_list, PROF) && TOK_TYPE != TK_FD)
		return (0);
	return (1);
}
